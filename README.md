...
TED Screen Editor is an editor to create text based screens for the Commodore 128 TED 80 column mode. It fully supports using user defined character sets and allows the use of two 256 character character sets at the same time.
...
- Supports all the attribute values the TED offers, blink, underline, reverse and alternate character set. For example the reverse attribute removes the need to have a reverse copy of every character set stored in the upper 128 positions of the character set, creating more positions that can be freely used to design own graphics.
- 64KiB TED memory only: An undo/redo system is included with up to 40 positions to go back or forward (depending on the size of the change in characters). As this uses a lot of memory, this feature is only available if 64 KiB of TED memory is available (such as in the C128DCR).
...
Mount the TEDSE disk image. Choose the .D64, .D71 or .D81 image depending on what is supported on the hardware or emulator you use. The program will run the same from all three images, but the .D71 and especially the .D81 will be providing faster load times and more available free storage space for your screens if you want to save your work in this same image. The disk can be mounted from any valid device ID number.

Run the program by entering RUN "TEDSE",U\<device number\> (or use other methods like JiffyDOS shortcodes or your favorite file browser). The TEDse file (with PRG file type) is the executable that should be started, the other files on the disk are supporting binaries, system fonts, title screen or help screens.

Description of contents of the disk image:

- TEDSE                     (PRG):      Main executable
- TEDSE.MACO                (PRG):      Machine code routines to be loaded in low memory
- TEDSE.FALT                (PRG):      System font for alternative charset
- TEDSE.FSTD                (PRG):      System font for standard charset
...
- TEDSE2PRG.MAC             (PRG):      Machine code helper routines of TEDSE2PRG
...
Only available if 64 KiB TED memory is detected: This option toggles if the Undo system is enabled or not. Default is enabled, but if preferred for speed, Undo can be disabled here.
...
TEDSE2PRG is a separate utility to create an executable program file for the C128 of a TEDSE project. Only 80x25 screens are supported at the moment, but redefined character sets are supported.

This is a seperate utility which can not be started from the TEDSE main program but has to be started seperately by loading the TEDSE2PRG file from disk with for example RUN"TEDSE2PRG",U(device number).
...
Follow the on screen instructions for selecting the input file and the output file name. Input file should be a TEDSE project file in 80x25 characters, with the associated screen and characterset files on the same disk/location.
...
|2000|07D0|Start of 48 byte padding. This is used to place a TEDSE version signature
|2048|0800|Start of attribute data using TED [attribute codes](#attribute-code-reference).
...
|Width * Height|Start of 48 byte padding. This is used to place a TEDSE version signature
|(Width * Height)+48|Start of attribute data using TED [attribute codes](#attribute-code-reference).
...
-   C128 Programmers Reference Guide: For the basic TED register routines and TED code inspiration

    http://www.zimmers.net/anonftp/pub/cbm/manuals/c128/C128_Programmers_Reference_Guide.pdf

-   Scott Hutter - TED Core functions inspiration:

    https://github.com/Commodore64128/TED_gui/blob/master/src/TED_core.c

    (used as starting point, but changed to inline assembler for core functions, added TED wait statements and expanded)
...
-   Tips and Tricks for C128: TED

    http://commodore128.mirkosoft.sk/TED.html
